...
memoria_charlas = {}
bloqueos_usuario: dict[str, asyncio.Lock] = {}
ultimo_mensaje: dict[str, tuple[str, datetime]] = {}


def obtener_lock(identificador: str) -> asyncio.Lock:
    """Crea o recupera un candado (Lock) único para cada usuario."""
    if identificador not in bloqueos_usuario:
        bloqueos_usuario[identificador] = asyncio.Lock()
    return bloqueos_usuario[identificador]
...
async def recibir_mensaje(datos: Request):
    ...
    # 4. Bloqueo de concurrencia y llamada a la IA
    async with obtener_lock(identificador):
        anterior = ultimo_mensaje.get(identificador)
        ahora = datetime.now()
        
        # Filtro antidobleteo (8 segundos)
        if anterior and anterior[0] == texto_usuario and (ahora - anterior[1]).total_seconds() < 8:
            print(f"Duplicado ignorado de {identificador}: {texto_usuario!r}")
            return {"respuesta_servidor": ""}

        # Actualizamos el último mensaje recibido
        ultimo_mensaje[identificador] = (texto_usuario, ahora)
        ...
        try:
            # Límite de 12 segundos para la IA
        ...
        # Guardamos la respuesta del asistente en memoria
        memoria_charlas[identificador].append(
